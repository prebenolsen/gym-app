...
phone. The backend must be publicly reachable **over HTTPS** (an HTTP backend is
blocked as mixed content from an HTTPS page) and must allow the site origin in its
CORS config.
...
| Variable | `EXPO_PUBLIC_API_BASE_URL` | public HTTPS URL of the `backend/` deployment |

3. Add the Pages origin (`https://<user>.github.io`) to the Supabase redirect URLs
   and the backend's CORS allowlist.

The workflow fails fast with a clear message if any of the three is missing, or if
the API URL still points at `localhost`.
